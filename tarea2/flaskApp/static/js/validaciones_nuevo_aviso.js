...
console.log("fdsdkjfjdsfsdfjl")

const validarForm = () => {
    // Region
    const validadorRegion = (region_) => {
        if (region_.options[region_.selectedIndex].text === "Seleccione una región") {
            return false;
        }
        return true
    }
    ...
    // Comuna
    const validadorComuna = (comuna_) => {
        if (comuna_.options[comuna_.selectedIndex].text == "Seleccione una comuna") {
            return false;
        }
        return true;
    }
    ...
    const validadorSector = (sector) => {
        ...
        return sector && sector.length <= 100;
    }
    ...
    // Fono XXX.XXXXXXXX (corregido)
    const validadorFono = (fono) => {
        if (!fono) return true;
        if (fono.length > 12) return false;
        const formato = ["0123456789", "0123456789", "0123456789", ".", "0123456789", "0123456789", "0123456789", "0123456789", "0123456789", "0123456789", "0123456789", "0123456789"]
        for (let i = 0; i < 12; i++) {
            const valorFono = fono[i];
            if (!formato[i].includes(valorFono)) {
                return false;
            }
        }
        return true;
    ...
    //Método contacto
    const validadorMContacto = (mContacto) => {
        if (mContacto.options[mContacto.selectedIndex].text === "Seleccione un método de contacto") {
            return false;
        }
        return true;
    }
    ...
    const validadorTipo = (tipo) => {
        if (tipo.options[tipo.selectedIndex].text === "Seleccione el tipo de su mascota") {
            return false;
        }
        return true;
    }
    ...
    //UNidad de medida
    const validadorUnidadDeMedida = (UM) => {
        if (UM.options[UM.selectedIndex].text === "Seleccione la unidad de medida de la edad") {
            return false;
        }
        return true;
    }
    ...
    const validadorFecha = (fecha) => {
        // Transformamos la fecha a un tipo date para lograr bien la comparación.
        const fecha_input = new Date(fecha);
        const fecha_actual = new Date();
        const fecha_actual_mas_3_horas = new Date(fecha_actual.getTime()+3 * 60 * 60 * 1000);
        if (fecha_input < fecha_actual_mas_3_horas) {
            return false;
        } 
        return true;
    }
    ...
    // Además se hace la acción de que hayan varios contactos
    ...
    const tipo = document.getElementById("tipo_mascota");
    const cantidad = document.getElementById("cantidad_mascota")
    const edad = document.getElementById("cantidad_mascota");
    const UM = document.getElementById("u_medida");
    const fecha = document.getElementById("fecha_disponible")
    const mContacto = document.getElementById("metodo_contacto")

    infoValida = true;
    msg = "";

    // Region
    if (!validadorRegion(region)) {
        infoValida = false;
        msg += "- Región no seleccionada, selecciona una. \n";
    ...
    // Comuna
    if (!validadorComuna(comuna)) {
        infoValida = false;
        msg += "- Comuna no seleccionada, selecciona una. \n";
    ...
    if (!validadorSector(sector.value)) {
        infoValida = false;
        msg += "- La sección sector tiene un límite de 100 caracteres, Intenta otra vez. \n";
    ...
    if (!validadorNombre(nombre.value)) {
        infoValida = false;
        msg += "- La sección nombre tiene un mínimo de 3 caracteres y un máximo de 100, Intenta otra vez. \n";
        nombre.style.borderColor = "red";
    } else {
        nombre.style.borderColor = "";
    }

    if (!validadorEmail(mail.value)) {
        infoValida = false;
        msg += "- El mail introducido no cumple con el formato mail. Intenta otra vez. \n";
        mail.style.borderColor = "red";
    } else {
        mail.style.borderColor = "";
    }

    if (!validadorFono(fono.value)) {
        infoValida = false;
        msg += "- Tu número de telefono no cumple con el formato NNN.NNNNNNNN, Intenta otra vez. \n";
        fono.style.borderColor = "red";
    } else {
        fono.style.borderColor = "";
    }

    if (!validadorTipo(tipo)) {
        infoValida = false;
        msg += "- Por favor escoge el tipo de tu animal. \n";
        tipo.style.borderColor = "red";
    } else {
        tipo.style.borderColor = "";
    }

    if (!validadorEdad(edad.value)) {
        infoValida = false;
        msg += "- La edad indicada es incorrecta. \n";
        edad.style.borderColor = "red";
    } else {
        edad.style.borderColor = "";
    }

    if (!validadorCantidad(cantidad.value)) {
        infoValida = false;
        msg += "- La cantidad indicada es incorrecta. \n"
        cantidad.style.borderColor = "red";
    } else {
        cantidad.style.borderColor = "";
    }

    if (!validadorUnidadDeMedida(UM)) {
        infoValida = false;
        msg += "- Por favor indica la unidad de medida de la edad de tu animal. \n";
        UM.style.borderColor = "red";
    } else {
        UM.style.borderColor = "";
    }

    if (!validadorFecha(fecha.value)) {
        infoValida = false;
        msg += "- La fecha escogida debe ser al menos 3 horas futuras a tu fecha actual. \n";
        fecha.style.borderColor = "red";
    } else {
        fecha.style.borderColor = "";
    }

    if (!validadorMContacto(mContacto)) {
        infoValida = false;
        msg += "- Por favor escoge un método de contacto \n";
        mContacto.style.borderColor = "red";
    } else {
        mContacto.style.borderColor = "";
    }

    if (infoValida == false) {
        alert(msg)
    } else {
        alert("¡La información proporcionada es válida!")
    }

}


let submitBtn = document.getElementById("boton_agregar_aviso")
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const year = mas3Horas.getFullYear();
    const month = String(mas3Horas.getMonth() + 1).padStart(2, "0");
    const day = String(mas3Horas.getDate()).padStart(2, "0");
    const hours = String(mas3Horas.getHours()).padStart(2, "0");
    const minutes = String(mas3Horas.getMinutes()).padStart(2, "0");

    const valor = `${year}-${month}-${day}T${hours}:${minutes}`;

    // Prellenar y asignar como mínimo
    fechaInput.value = valor;
    fechaInput.min = valor;
});

// Agregar nuevo slot de contacto al presionar agregar otro método de contacto
const containerContactos = document.getElementById("mContacto_div");
...
btnAgregarContacto.addEventListener("click", () => {
    ...
    if (countBloques >= 5) {
        alert("Solo puedes agregar hasta 5 métodos de contacto.");
        return;
    } else {
    const nuevo = document.createElement("div");
    nuevo.classList.add("contacto");
    nuevo.innerHTML = `
        <select name="metodo_contacto[]" class="metodo_contacto">
        <option value="none">Seleccione un método de contacto</option>
        ...
        <input type="text" name="contacto_detalle[]" placeholder="Ingrese su contacto" minlength="4" maxlength="50" required>
    `;
    containerContactos.appendChild(nuevo);
    countBloques+=1;
}
    });

const containerFotos = document.getElementById("foto_div");
const btnAgregarFoto = document.getElementById("boton_agregar_foto");
let countFotos = 1;

btnAgregarFoto.addEventListener("click", () => {
    if (countFotos >= 5){
        alert("Solo puedes agregar hasta 5 fotos de tu animalito.");
        return;
    } else {
        const nuevoFile = document.createElement("div");
        nuevoFile.classList.add("foto");
        // Quitamos el id duplicado y usamos name con array para que se puedan enviar varias
        nuevoFile.innerHTML = `<input type="file" name="foto_mascota[]" required>`;
        containerFotos.appendChild(nuevoFile);
        countFotos += 1;
    }
});

const btnEnviar = document.getElementById("boton_agregar_aviso");
const confirmContainer = document.getElementById("confirmContainer");
const form = document.getElementById("form_aviso_adopcion");

btnEnviar.addEventListener("click", () => {
    if (!infoValida) {
        alert("La información proporcionada no es válida y/o contiene errores. Por favor vuelve al formulario")
    } else {
        const mensaje = document.createElement("p");
    mensaje.textContent = "¿Está seguro que desea agregar este aviso de adopción?";
    confirmContainer.appendChild(mensaje);
    ...
    btnSi.addEventListener("click", () => {
        ...
        const containerBtnPortada = document.getElementById("boton_portada");
        const btnPortada = document.createElement("button");
        btnPortada.textContent = "Volver a la página principal";
        btnPortada.type = "button";
        containerBtnPortada.appendChild(btnPortada)
        btnPortada.addEventListener("click", () => {
            window.location.href = "portada.html";
});
    });
    confirmContainer.appendChild(btnSi);
    ...
    confirmContainer.appendChild(btnNo);
    }
});
